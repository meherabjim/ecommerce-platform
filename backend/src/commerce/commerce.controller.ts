...
import { AssignDeliveryDto,DeliveryStatusDto } from './dto/delivery.dto';
...
export class CommerceController{
 ...
 @Get('orders/public/track') track(@Query('orderNumber')orderNumber:string,@Query('phone')phone:string){return this.s.publicTrackOrder(orderNumber,phone)}
 ...
 @Post('me/orders/:id/cancel')@UseGuards(JwtAuthGuard)cancelMyOrder(@CurrentUser()u:any,@Param('id')id:string,@Body()d:CancelOrderDto){return this.s.cancelByCustomer(id,u.id,d.reason)}
 @Post('me/orders/:id/reorder')@UseGuards(JwtAuthGuard)reorder(@CurrentUser()u:any,@Param('id')id:string){return this.s.reorder(id,u.id)}
 @Get('admin/orders')@UseGuards(JwtAuthGuard,RolesGuard)@Roles(UserRole.ADMIN)orders(){return this.s.allOrders()}
 @Get('admin/orders/:id')@UseGuards(JwtAuthGuard,RolesGuard)@Roles(UserRole.ADMIN)adminOrder(@Param('id')id:string){return this.s.orderDetails(id,undefined,true)}
 ...
 @Patch('admin/orders/:id/payment-status')@UseGuards(JwtAuthGuard,RolesGuard)@Roles(UserRole.ADMIN)paymentStatus(@CurrentUser()u:any,@Param('id')id:string,@Body()d:UpdatePaymentStatusDto){return this.s.adminUpdatePaymentStatus(id,d.status,u.id,d.note)}
