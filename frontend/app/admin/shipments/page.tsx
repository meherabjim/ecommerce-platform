...
export default function ShipmentsPage(){
  ...
  return <AdminShell>
    <div className="flex flex-col justify-between gap-4 xl:flex-row xl:items-end"><div><p className="text-xs font-black uppercase tracking-[.18em] text-slate-400">SRS fulfillment operations</p><h1 className="mt-2 text-3xl font-black tracking-tight">Courier shipments</h1><p className="mt-2 text-sm text-slate-500">Consignment, tracking events, external provider records and COD settlement reconciliation.</p></div><label className="flex items-center gap-2 rounded-xl border bg-white px-4"><Search size={16}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search shipment..." className="py-3 outline-none"/></label></div>
