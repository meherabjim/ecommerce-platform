...
export default function Navbar(){
  ...
  return <>
    <div className="sticky top-0 z-[70] border-b border-[#284a6a] bg-[#0d1b2a] text-white shadow-sm">
      <div className="mx-auto flex min-h-8 max-w-7xl items-center gap-3 px-4 sm:px-5">
    ...
    <header className="sticky top-11 z-50 border-b border-[#456785] bg-[#18314b]/98 text-white shadow-[0_10px_26px_rgba(0,0,0,.22)] backdrop-blur-xl">
      <div className="mx-auto flex min-h-[60px] max-w-7xl items-center gap-3 px-4 sm:px-5">
        <Link href="/" className="flex shrink-0 items-center gap-3">
          {logoUrl?<img src={logoUrl} alt={storeName} className="h-9 w-9 rounded-xl object-cover"/>:<span className="grid h-9 w-9 place-items-center rounded-full bg-[#ff9900] text-lg font-black text-[#17202b]">{storeName.charAt(0).toUpperCase()}</span>}
          <div className="hidden sm:block"><p className="text-[15px] font-black leading-none text-white">{storeName}</p><p className="mt-1 text-[9px] font-bold uppercase tracking-[.16em] text-sky-300">{tagline}</p></div>
        </Link>

        <div className="relative hidden lg:block">
          <button onClick={()=>setCategoryOpen(v=>!v)} className="flex h-9 items-center gap-2 rounded-full border border-[#4b6b89] bg-[#223f5c] px-3.5 text-xs font-black text-white transition hover:-translate-y-0.5 hover:border-[#38bdf8] hover:text-[#7dd3fc]">{t('categories')}<ChevronDown size={15}/></button>
        ...
        <div className="flex items-center gap-1.5">
          {ready&&user?.role==='CUSTOMER'&&<div className="relative hidden sm:block">
            <button
              ...
              className="relative grid h-9 w-9 place-items-center rounded-xl text-slate-100 hover:bg-[#294866] hover:text-sky-300"
            >
              <Bell size={20}/>
